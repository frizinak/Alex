...
Editor.page_click = function (e) {
  ...
  if (Editor.currentPage.hasOwnProperty('canSave') && Editor.currentPage['canSave'] !== true && Editor.currentPage['canSave'] !== 'true') {
    notice(text.ReadOnly, 1, 0, Editor.currentPageName);
    return false;
  }
  ...
  Editor.generate_form(Editor.currentPage, Editor.currentPageTpl);
  $(window).resize();
  return false;
};

Editor.generate_form = function (page, tpl) {
  var i, key, pageData = page.tplData , tplData = tpl.tplData, input, inputDef, isLocked , editorForm = $('#editor-form'), parentSelectString, parentIgnoredChildren, checked;
  if (!page.hasOwnProperty('lockedInputs') || !page['lockedInputs'].indexOf || page['lockedInputs'].indexOf('pagename') == -1) {
  ...
  if (!page.hasOwnProperty('lockedInputs') || !page['lockedInputs'].indexOf || page['lockedInputs'].indexOf('parent') == -1) {
  ...
  for (key in tplData) {
    if (!tplData.hasOwnProperty(key) || (page.hasOwnProperty('lockedInputs') && page['lockedInputs'].indexOf && page['lockedInputs'].indexOf(key) > -1)) {
      continue;
    }
    inputDef = tplData[key];
    input = "";
    switch (inputDef.type) {
    case 'text':
      input = '<label data-title="' + inputDef.description + '">' + inputDef.label + '</label><br/>' +
        '<input class="tplvar" name="' + key + '" type="text" value="' + htmlentities(pageData[key]) + '" /><br/>';
      break;
    case 'richtextarea':
      input = '<label data-title="' + inputDef.description + '">' + inputDef.label + '</label><br/>' +
        '<textarea class="tplvar rich" name="' + key + '" rows="8" cols="40" >' + htmlentities(pageData[key]) + '</textarea><br/>';
      break;
    case 'textarea':
      input = '<label data-title="' + inputDef.description + '">' + inputDef.label + '</label><br/>' +
        '<textarea class="tplvar" name="' + key + '" rows="8" cols="40" >' + htmlentities(pageData[key]) + '</textarea><br/>';
      break;
    case 'checkbox':
      checked = ((pageData === undefined || pageData[key] === undefined || $.isPlainObject(pageData[key])) && inputDef['default'] === inputDef['values'][0]) || pageData[key] === inputDef['values'][0];
      input = '<label data-title="' + inputDef.description + '">' + inputDef.label + '</label>'
      input += '<span class="checkbox ' + (checked ? ' checked' : '') + '"></span>'
      input += '<input class="tplvar" name="' + key + '" type="checkbox" ' + (checked ? 'checked="checked"' : '') + ' /><br/>';
      break;
    case 'radio':
      input = '<label data-title="' + inputDef.description + '">' + inputDef.label + '</label><div class="rdgroup">';
      for (i = 0; i < inputDef.values.length; i++) {
        checked = ((pageData === undefined || pageData[key] === undefined || $.isPlainObject(pageData[key])) && inputDef['default'] === inputDef['values'][i]) || pageData[key] === inputDef['values'][i];
        input += '<label class="rdlbl">' + inputDef.labels[i] + '</label>';
        input += '<span class="radio' + (checked ? ' checked' : '') + '"></span>'
        input += '<input class="tplvar" type="radio" value="' + inputDef.values[i] + '" name="' + key + '" ' + (checked ? 'checked="checked"' : '') + ' /><br/>';
      }
      input += '</div>';
      break;
    default:
      if (App.plugins.hasOwnProperty(inputDef.type)) {
        var plugin = new App.plugins[inputDef.type]();
        input = '<span class="pluginContainer" id="' + key + '">' + plugin.generate_form(inputDef, pageData[key], key) + '</span>';
  ...
  for (i = 0; i < Editor.plugins.length; i++) {
    Editor.plugins[i].generated_form();
  }
  if (!page.hasOwnProperty('lockedInputs') || !page['lockedInputs'].indexOf || page['lockedInputs'].indexOf('cache') == -1) {
  ...
  if (!page.hasOwnProperty('lockedInputs') || !page['lockedInputs'].indexOf || page['lockedInputs'].indexOf('menu') == -1) {
  ...
  if (!page.hasOwnProperty('lockedInputs') || !page['lockedInputs'].indexOf || page['lockedInputs'].indexOf('publish') == -1) {
  ...
  if (!page.hasOwnProperty('canDelete') || page['canDelete'] === true || page['canDelete'] === 'true') {
    editorForm.append('<a id="removepage" href="#">' + text.DeleteBtn + '</a>');
  }
  if (!page.hasOwnProperty('canSave') || page['canSave'] === true || page['canSave'] === 'true') {
    editorForm.append('<a id="submiteditform" href="#">' + text.SaveBtn + '</a>');
  }
...
Editor.submit_editor = function () {
  ...
  var pageName = Editor.currentPageName;
  ...
  if ($('.published').is(':checked')) {
    if (Editor.currentPageStructureIndex === -1) {
      cmx.pageTree.pagesList.push(pageName);
    } else {
      cmx.pageTree.pagesList[Editor.currentPageStructureIndex] = pageName;
    }
  } else if ($('.published').size() > 0 && Editor.currentPageStructureIndex !== -1) {
